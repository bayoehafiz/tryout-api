...
export const showParticipant = ({ params }, res, next) =>
  Test.findById(params.id, { users: 1 })
  .then(notFound(res))
  .then((test) => test ? test.view() : null)
  .then(success(res))
  .catch(next)
...
export const submitTestResult = ({ bodymen: { body }, params }, res, next) =>
  Test.findById(params.id)
  .then(notFound(res))
  .then((result) => {
    if (!result) return null
    var currentUsers = result.user
    // var newData = 
  })
  .then((test) => test ? Object.assign(test, body).save() : null)
  .then((test) => test ? test.view(true) : null)
